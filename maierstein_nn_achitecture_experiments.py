...
def PINN(training_set, alpha,epochs,layer_array,lr):
    ...
    filename = f"./model_weights1/pinn_NumOfTrPts-{len(training_set)}_LayerArray-{layer_array}_alpha-{alpha}_epochs-{epochs}_DATE-{year}-{month}-{day}-{hour}-{minute}"
    ...
    ax.set_ylim([0.0, 10])
    plt.plot(loss_plot)
    plt.savefig("./training_loss_images1/"+ f"pinn_NumOfTrPts-{len(training_set)}_LayerArray-{layer_array}_alpha-{alpha}_epochs-{epochs}_DATE-{year}-{month}-{day}-{hour}-{minute}"+".png")
    plt.clf()
...
def hybrid(training_set, layer_array, lr, epochs): 
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=lr)
    loss_fn = torch.nn.MSELoss()
    my_training_object = ha.TrainingLiLin(NN=model, optimizer=optimizer, loss_fn=loss_fn, epochs=epochs, training_points=training_set)
    ...
    x = datetime.today()
    year, month, day, hour, minute = x.year, x.month, x.day, x.hour, x.minute
    filename = f"./model_weights1/hyrbid_NumOfTrPts-{len(training_set)}_LayerArray-{layer_array}_epochs-{epochs}_DATE-{year}-{month}-{day}-{hour}-{minute}"
    torch.save(model.state_dict(), filename)
    ...
    ax.set_ylim([0.0, 10])
    plt.plot(loss_plot)
    plt.savefig("./training_loss_images1/"+ f"hyrbid_NumOfTrPts-{len(training_set)}_LayerArray-{layer_array}_epochs-{epochs}_DATE-{year}-{month}-{day}-{hour}-{minute}"+".png")
    plt.clf()
...
if __name__=="__main__":
    ...
    T5 = torch.load("./training_data/fem_ellipse_with_AB_4888_pts.pt")
    T6 = torch.load("./training_data/uniformsample11250pts.pt")
    T5.requires_grad_(True)
    T6.requires_grad_(True)
    T1b = torch.load("./training_data/delta_rarified-AB_REMOVEDnum_pts-956-delta-0.05-bumps-1000-tau-1000-h_param-0.2-w_param-0.025-deltat-0.001-datetime-2022-8-3-16-12.pt")
    T2b = torch.load("./training_data/delta_rarified-AB_REMOVEDnum_pts-17255-delta-0.01-bumps-1000-tau-1000-h_param-0.2-w_param-0.025-deltat-0.001-datetime-2022-8-3-16-13.pt")
    T3b = torch.load("./training_data/delta_rarified-AB_REMOVEDnum_pts-31547-delta-0.007-bumps-1000-tau-1000-h_param-0.2-w_param-0.025-deltat-0.001-datetime-2022-8-3-16-19.pt")
    T4b = torch.load("./training_data/delta_rarified-AB_REMOVEDnum_pts-54636-delta-0.005-bumps-1000-tau-1000-h_param-0.2-w_param-0.025-deltat-0.001-datetime-2022-7-29-11-25.pt")
    ...
    training_sets = [T1, T2, T3, T4, T5, T6, T1b, T2b, T3b, T4b, T5b, T6b]

    # for T in training_sets:
    #     PINN(T, 10.0, 100, [2,20,20,1], 1e-3)
    #     hybrid(T,[2,20,20,1],1e-3, 100)

    pinn_combos = [[T,10.0,100,[2,20,20,1],1e-3] for T in training_sets]
    hybrid_combos = [[T,[2,20,20,1],1e-3,100] for T in training_sets]

    with Pool() as pool: 
        pool.starmap(PINN, pinn_combos)
        pool.starmap(hybrid, hybrid_combos)
